...
const BOT_TOKEN = process.env.BOT_TOKEN || '8412020501:AAHpZfTzLRN9M03iYOa19sJPbYqdFgCtljM';
...
const translations = {
    uz: {
        ...
        back: '⬅️ Orqaga',
        reply_to_user: '✉️ Foydalanuvchiga javob yozing (user_id: ',
        reply_sent: '✅ Javob yuborildi!',
        enter_reply: 'Javob matnini kiriting:'
    },
    ru: {
        ...
        back: '⬅️ Назад',
        reply_to_user: '✉️ Ответ пользователю (user_id: ',
        reply_sent: '✅ Ответ отправлен!',
        enter_reply: 'Введите текст ответа:'
    },
    en: {
        ...
        admin_panel: '👨‍💻 Admin panel',
        back: '⬅️ Back',
        reply_to_user: '✉️ Reply to user (user_id: ',
        reply_sent: '✅ Reply sent!',
        enter_reply: 'Enter reply text:'
    }
};
...
bot.on('text', async (ctx) => {
    ...
    if (ctx.session?.waitingForMovieCode) {
        ...
        db.get('SELECT url, channel_username, message_id FROM movies WHERE code = ?', [text], async (err, movie) => {
            ...
            try {
                ...
                if (movie.url) {
                    const urlMatch = movie.url.match(/t\.me\/([^\/]+)\/(\d+)/);
                    if (urlMatch) {
                        channel = `@${urlMatch[1]}`;
                ...
                if (!channel && movie.channel_username) {
        ...
        try {
            await bot.telegram.sendMessage(ADMIN_ID, 
                ...
                Markup.inlineKeyboard([
                    [Markup.button.callback('✉️ Javob yozish', `reply_${userId}_${messageId}`)]
                ]));
        } catch (error) {
    ...
    if (ctx.session?.waitingForBroadcast && userId === ADMIN_ID) {
        ...
        db.all('SELECT user_id FROM users WHERE user_id != ?', [ADMIN_ID], async (err, users) => {
            if (err || !users) return;
            ...
            for (const user of users) {
                try {
                    ...
                    console.error(`Broadcast error to ${user.user_id}:`, error);
                    failCount++;
                }
            }
    ...
    // Admin reply to user
    if (ctx.session?.waitingForAdminReply && userId === ADMIN_ID) {
        ctx.session.waitingForAdminReply = false;
        const replyUserId = ctx.session.replyUserId;
        delete ctx.session.replyUserId;

        try {
            await bot.telegram.sendMessage(replyUserId, 
                `✉️ Admin javobi:\n\n${text}`);
            ctx.reply(translations[lang].reply_sent);
            showAdminPanel(ctx);
        } catch (error) {
            console.error('Send reply error:', error);
            ctx.reply('❌ Javob yuborishda xatolik!');
            showAdminPanel(ctx);
        }
        return;
    }
...
bot.action(/reply_(\d+)_(\d+)/, async (ctx) => {
    ...
    const lang = await getUserLanguage(ADMIN_ID);
    ctx.session = {
        waitingForAdminReply: true,
        replyUserId: userId
    };
    
    ctx.editMessageText(translations[lang].reply_to_user + `${userId}):`, Markup.inlineKeyboard([
...
function showAdminPanel(ctx) {
    ...
    const buttons = [
        ...
        [Markup.button.callback('🔒 Majburiy obuna', 'admin_channels')],
    ...
    if (ctx.updateType === 'callback_query') {
        ...
        ctx.reply('👨‍💻 Admin panel:', Markup.inlineKeyboard(buttons));
    }
}
...
bot.action('admin_channels', async (ctx) => {
    ...
    const buttons = [
        ...
        [Markup.button.callback('➖ Kanal o‘chirish', 'admin_channel_remove')],
        [Markup.button.callback('📋 Kanallar ro‘yxati', 'admin_channel_list')],
        [Markup.button.callback('⬅️ Orqaga', 'admin_panel')]
    ];
    
    ctx.editMessageText('🔒 Majburiy obuna kanallari boshqaruvi:', Markup.inlineKeyboard(buttons));
});
...
bot.action('admin_channel_add', async (ctx) => {
    ...
    ctx.editMessageText(translations[lang].enter_channel, Markup.inlineKeyboard([
        [Markup.button.callback(translations[lang].cancel, 'admin_channels')]
    ]));
});
...
// Channel list
bot.action('admin_channel_list', async (ctx) => {
    if (ctx.from.id !== ADMIN_ID) return;
    
    db.all('SELECT username FROM channels WHERE is_required = 1', (err, rows) => {
        if (err || !rows || rows.length === 0) {
            ctx.editMessageText('📋 Majburiy kanallar ro‘yxati bo‘sh.', Markup.inlineKeyboard([
